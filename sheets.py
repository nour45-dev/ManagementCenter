...
def get_statistics_updated() -> dict:
    """
    إحصائيات شاملة - كل الطلاب + اللي عندهم مواد ومدرسين
    """
    try:
        sheet = connect_to_sheet()
        all_data = sheet.get_all_records()
...
def _parse_entries(teachers_str: str) -> list:
    ...
    بتدعم:
      "عربي/ا/ احمد عبد القادر | كيمياء/ا/ محمد صلاح"
      "عربي/احمد وسيد | تاريخ/ا/ رضا صلاح"
      "فلسفة/كلهم"
      "عربي::ا/ احمد"  (فورمات جديد)
    """
    results = []
...
def get_teacher_stats(teacher_name: str = None) -> dict | list:
    """
    إحصائيات المدرسين مع تكسير بالسنة والتخصص
    """
    try:
        sheet = connect_to_sheet()
        all_data = sheet.get_all_records()
        ...
        for student in all_data:
            ...
            for subject, teacher in _parse_entries(teachers_str):
                # لو المدرس "كلهم" نتخطاه في الإحصائيات
